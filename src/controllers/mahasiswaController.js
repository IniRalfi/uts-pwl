...
export const index = async (c) => {
  const data = await model.getAll(); // Update: Tambah await
...
export const store = async (c) => {
  ...
  await model.create({ // Update: Tambah await
    nama: body.nama,
    nim: body.nim,
  });
...
export const editForm = async (c) => {
  ...
  const data = await model.getById(id); // Update: Tambah await
...
export const updateData = async (c) => {
  ...
  await model.update(id, { // Update: Tambah await
    nama: body.nama,
    nim: body.nim,
  });
...
export const destroy = async (c) => {
  const id = c.req.param("id");
  await model.remove(id); // Update: Tambah await
